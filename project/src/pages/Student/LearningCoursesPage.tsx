...
const LearningCoursesPage: React.FC = () => {
  ...
  // ✅ Charger les cours + inscriptions
  useEffect(() => {
  ...
  // ✅ Vérifie si l'utilisateur est déjà inscrit à un cours
  const isCourseEnrolled = (courseId: number): boolean => {
    return enrollments.some((enr) => enr.learning_course === courseId);
  };
            ...
            {learningCourses.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {learningCourses.map((course) => {
                  ...
                  return (
                    <div key={`learning-${course.id}`} className="relative">
                      <CourseCard
                        course={course}
                        isEnrolled={enrolled}
                        onEnroll={() => enroll(course.id, "learning")}
